...
SELECT
    ...
    sex_male_moe,
    CASE
        WHEN sex_total = 0 THEN NULL
        ELSE CAST(sex_male AS BIGNUMERIC) / CAST(sex_total AS BIGNUMERIC)
    END AS pct_sex_male,
    (
    ...
    CASE
        ...
        ELSE CAST(
            (
                ...
                sex_male_85_and_over +
                sex_male_85_and_over_moe
            ) AS BIGNUMERIC) / CAST(sex_total AS BIGNUMERIC)
    END AS pct_male_voting_age_population,
    sex_female,
    sex_female_moe,
    CASE
        WHEN sex_total = 0 THEN NULL
        ELSE CAST(sex_female AS BIGNUMERIC) / CAST(sex_total AS BIGNUMERIC)
    END AS pct_sex_female,
    (
    ...
    END AS pct_female_voting_age_population,
    race_white_total,
    race_white_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_white_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_white,
    race_black_total,
    race_black_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_black_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_black,
    race_asian_total,
    race_asian_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_asian_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_asian,
    race_pacific_islander_total,
    race_pacific_islander_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_pacific_islander_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_pacific_islander,
    race_other_total,
    race_other_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_other_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_other,
    race_two_or_more_total,
    race_two_or_more_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(race_two_or_more_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_race_two_or_more,
    hispanic_total,
    hispanic_total_moe,
    CASE
        WHEN total_population = 0 THEN NULL
        ELSE CAST(hispanic_total AS BIGNUMERIC) / CAST(total_population AS BIGNUMERIC)
    END AS pct_hispanic
