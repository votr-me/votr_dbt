...
SELECT
    ...
    CASE 
        ...
        ELSE sex_male::float / sex_total::float 
    END as pct_sex_male,
    (
    ...
    CASE 
        ...
        ELSE (
            ...
            sex_male_85_and_over +
            sex_male_85_and_over_moe
        )::float / sex_total::float 
    END as pct_male_voting_age_population,
    sex_female,
    sex_female_moe,
    CASE 
        WHEN sex_total = 0 THEN Null
        ELSE sex_female::float / sex_total::float 
    ...
    CASE 
        ...
        ELSE race_white_total::float / total_population::float 
    END as pct_race_white,
    race_black_total,
    race_black_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE race_black_total::float / total_population::float 
    END as pct_race_black,
    race_asian_total,
    race_asian_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE race_asian_total::float / total_population::float 
    END as pct_race_asian,
    race_pacific_islander_total,
    race_pacific_islander_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE race_pacific_islander_total::float / total_population::float 
    END as pct_race_pacific_islander,
    race_other_total,
    race_other_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE race_other_total::float / total_population::float 
    END as pct_race_other,
    race_two_or_more_total,
    race_two_or_more_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE race_two_or_more_total::float / total_population::float 
    END as pct_race_two_or_more,
    hispanic_total,
    hispanic_total_moe,
    CASE 
        WHEN total_population = 0 THEN Null
        ELSE hispanic_total::float / total_population::float 
    END as pct_hispanic
